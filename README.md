...
| 팀원 | 담당 영역 | 주요 구현 내용 |
| --- | --- | --- |
| 정민주 | 산책 | 산책 시작·완료 화면, GPS 경로 기록, 시간·거리·페이스 표시, 사진 등록, 달력·상세·경로 화면과 백엔드 산책 API 연동 |
| 석종수 | 인증·마이페이지·반려견·그룹 | 이메일·카카오 로그인, JWT 인증 화면, 회원·프로필 이미지, 반려견 등록·수정·대표견, 그룹 생성·초대·관리·기록 공유 UI |
| 김태중 | 장소·관광공사 API | 관광공사 장소 데이터 연동, 주변·지역·키워드 조회, 출입 조건 분석, 반려견 맞춤 장소 필터, 즐겨찾기·신고·정책 이력 백엔드 기능 |
| 전민규 | 커뮤니티·지도 UI·Docker | React 초기 구조, 게시판·게시글·댓글·이미지 UI와 API 연결, 카테고리·작성자 수정·삭제, 지도·검색·필터 UI, Docker Compose·Nginx 구성 |
